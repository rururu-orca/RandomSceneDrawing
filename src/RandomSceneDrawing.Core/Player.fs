...
type Cmd<'player>(api: Api<'player>, player) =
    ...
    let teeError = TaskResult.teeError (ErrorMsg >> showInfomation)

    member _.ShowInfomation info = showInfomation info

    member _.PlayCmd() =
        Cmd.OfTask.perform (api.playAsync >> teeError) player (Finished >> Play)

    member _.PauseCmd() =
        Cmd.OfTask.perform (api.pauseAsync >> teeError) player (Finished >> Pause)

    member _.StopCmd() =
        Cmd.OfTask.perform (api.stopAsync >> TaskResult.teeError (ErrorMsg >> showInfomation)) player (Finished >> Stop)
