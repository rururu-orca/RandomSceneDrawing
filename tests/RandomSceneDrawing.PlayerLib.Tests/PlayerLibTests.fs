...
let mainApi: Main.Api<MediaPlayer> = {
    ...
    showInfomation = fun _ -> task { () }
    randomize =
        fun randomizeSource (player) (subPlayer) -> taskResult {
...
module IAsyncDisposable =
    ...
    let create onDispose =
        { new IAsyncDisposable with
            member _.DisposeAsync() = task { onDispose () } |> ValueTask
        }

    let ofDisposable (disposable: IDisposable) =
        { new IAsyncDisposable with
            member _.DisposeAsync() =
                task { disposable.Dispose() } |> ValueTask
        }
...
let playerLibTests =
    ...
    let playListPath =
        [| __SOURCE_DIRECTORY__; "TestPlayList.xspf" |]
        |> Path.Combine
        |> playListFilePath.Create

    let snapShot = Path.Combine [| __SOURCE_DIRECTORY__; "test.png" |]
    ...
    <| [
        ...
        testTask "can load Playlist" {
            ...
            Expect.equal MediaType.File actual "should work"

        }
        testTask "can randomize and snapshot" {

            let player = LibVLCSharp.initPlayer ()
            use _ = IAsyncDisposable.ofDisposable player
            let subPlayer = LibVLCSharp.initSubPlayer ()
            use _ = IAsyncDisposable.ofDisposable subPlayer
